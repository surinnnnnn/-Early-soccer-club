  ...
  async (req, res, next) => {
    ...
    try {
      ...
      for (let i = 1; i <= 3; i++) {
        ...
        let successRateGroup = goalProbabilityGroup * 100;

        // 우리팀 공격수 2명 이상 시 골성공 확률 +5
        const attackerCount = myAgentsStatsGroup.filter(
          (agent) => agent.position === "attacker",
        ).length;
        if (attackerCount >= 2) {
          successRateGroup += 5;
        }
        //상대 수비수 2명 이상 시 성공확률 -5
        const defenderCount = opponentAgentsStatsGroup.filter(
          (agent) => agent.position === "defender",
        ).length;
        if (defenderCount >= 2) {
          successRateGroup -= 5;
        }

        // 우리팀 미드필더 2명 이상 시 골성공 확률 +2.5
        const midfielderCount = myAgentsStatsGroup.filter(
          (agent) => agent.position === "midfielder",
        ).length;
        if (midfielderCount >= 2) {
          successRateGroup += 2.5;
        }
      ...
      const myRecord = await usersPrisma.users.findFirst({
        where: { user_id },
        select: { win: true, lost: true, match_count: true },
      });

      // 승률 표시
      const odds = (myRecord.win / myRecord.match_count) * 100;
      ...
      Object.keys(groups).forEach((groupKey) => {
        groups[groupKey].forEach((agent_id, index) => {
          ...
          const teamName = myteam.agent_ids.includes(agent_id)
            ? `[우리팀]` + myteam.team_name
            : `[상대팀]` + opponent.team_name;
          if (agent) {
      ...
      return res.status(200).json({
        ...
        result: result,
        odds_of_winning: `${odds.toFixed(2)}%`,
      });
    } catch (error) {
