...
router.post("/my_team", authMiddleware, async (req, res, next) => {
  try {
    ...
    if (team_name.length > 10) {
      return res
        .status(400)
        .json({ message: "10글자 이내 팀 이름만 사용 가능합니다." });
    }
    ...
    if (newTeam) {
      const members = await gamePrisma.agents.findMany({
        where: { agent_id: { in: newTeam.agent_ids || [] } },
        select: {
          agent_id: true,
          agent_name: true,
        },
...
router.get("/teams", async (req, res, next) => {
  try {
    ...
    const teamDataPromises = teams.map(async (team) => {
      // agent_ids 배열에 t서 agent의 정보를 조회
      const members = await gamePrisma.agents.findMany({
      ...
      //전적 정보 조회
      const rate = await usersPrisma.users.findFirst({
      ...
      let odds = 0;
      if (rate && rate.match_count > 0) {
        odds = (rate.win / rate.match_count) * 100;
      }
